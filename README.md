...
| fuzzy | 22/23 | 195/200 | 0 |

**995/1000 deterministic traces pass** — byte-exact state root match,
both chain and step modes. 0 silent errors.

### Remaining failures — 5 steps (fuzzy only)

All 5 failures are in `fuzzy` (random service profile, max 6 work items).
...
| Fix | Impact | Details |
|-----|-------:|---------|
| **Cross-service account fields** | +6 steps | `build-cross-service-accounts` was missing `creation-slot`, `last-accumulation-slot`, and `parent-service` fields. HC5 (ΩI) for other services returned zeros for these fields instead of actual values from δ. Contracts querying cross-service info got wrong `last_accum_slot` etc., producing incorrect storage writes. |
| **ARGS_SEGMENT mapping** | correctness | `argument-invoke` now maps arguments at `ARGS_SEGMENT` (0xFEFF0000) as read-only memory per GP A.8 / SPI convention, instead of allocating on the heap via `sbrk`. Ensures the guest's heap pointer is not affected by argument size. |
