...
are implemented in pure Common Lisp.

## Conformance

All published [test vectors](https://github.com/w3f/jamtestvectors) pass without error.
...
| minifuzz no\_forks (fuzz-v1) | **102/102 ✓** |
...
Every GP state component (τ η κ λ β ψ ρ ι γ α ϕ δ π χ ω ξ θ) is represented
as an immutable closure generated by a single macro, `define-state-closure`.
Each closure encapsulates its data, binary codec, and transition logic.
No mutable state, no CLOS.

All closures implement a uniform message-passing protocol:

- **`:transition`** accepts the relevant inputs for a given GP section and
  returns a new closure representing the post-transition state.
- **Transient fields** capture side-data produced during transition
  (e.g. R\* from ω, emitted validators from χ) and are queryable on the
  returned closure.
- **Multi-stage transitions** (`:transition-dagger`, `:transition-ddagger`)
  support components with intermediate states (δ†, ρ†, ρ‡).

Components are sovereign: each owns its encoding, decoding, and transition
logic. Orchestrators coordinate data flow between components but never
inspect or modify their internal state.

### Orchestration

The block-level transition Υ(σ, B) → σ' is implemented in `upsilon.lisp`,
which executes the 4-wave dependency graph defined in GP §4.2.1. The
accumulation phase (GP §12) is handled by `accumulate.lisp`, which
coordinates R\* extraction, per-service PVM execution, and privilege resolution.

`import-block` serves as the trust boundary: below it, all computation is
pure and deterministic; above it, environmental observations (wall-clock,
parent hash) are applied.

### PVM

The PVM interpreter (GP Appendix A) is written in pure Common Lisp.
Initialization precomputes basic-block start positions (ω̄) and a
skip-distance lookup table; individual instructions are decoded lazily
on first execution and cached for subsequent hits at the same program counter.

Host calls (GP Appendix B) are implemented in `src/jam-host/`, with
...
./scripts/test.sh safrole preimages         # multiple traces
...
├── import.lisp         Block import and validation boundary
...
scripts/                Shell entry points and SBCL loader
```

<details><summary>Bugs found and fixed during conformance testing</summary>
