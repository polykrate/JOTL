...
(definstruction :set-lt-u-imm (vm args)
  (set-reg vm (arg-ra args) (if (< (reg vm (arg-rb args)) (u64 (arg-imm args))) 1 0))
  :continue)
...
(definstruction :set-lt-s-imm (vm args)
  (set-reg vm (arg-ra args)
           (if (< (zn 8 (reg vm (arg-rb args))) (zn 8 (u64 (arg-imm args)))) 1 0))
  :continue)
...
(definstruction :shlo-l-imm-32 (vm args)
  (let* ((shift (mod (arg-imm args) 32))
         (result (logand (ash (reg vm (arg-rb args)) shift) #xFFFFFFFF)))
    (set-reg vm (arg-ra args) (sign-extend result 4))
    :continue))
...
(definstruction :shlo-r-imm-32 (vm args)
  (let* ((shift (mod (arg-imm args) 32))
         (v32 (logand (reg vm (arg-rb args)) #xFFFFFFFF))
...
(definstruction :shar-r-imm-32 (vm args)
  (let* ((shift (mod (arg-imm args) 32))
    ...
    (set-reg vm (arg-ra args) (zn-inv 8 shifted))  ; Z₈⁻¹: signed → u64
    :continue))
...
(definstruction :neg-add-imm-32 (vm args)
  (let ((result (logand (- (arg-imm args) (reg vm (arg-rb args))) #xFFFFFFFF)))
    (set-reg vm (arg-ra args) (sign-extend result 4))
    :continue))
...
(definstruction :shlo-l-imm-alt-32 (vm args)
  (let* ((shift (mod (reg vm (arg-rb args)) 32))
         (result (logand (ash (u64 (arg-imm args)) shift) #xFFFFFFFF)))
    (set-reg vm (arg-ra args) (sign-extend result 4))
    :continue))
...
(definstruction :shlo-r-imm-alt-32 (vm args)
  (let* ((shift (mod (reg vm (arg-rb args)) 32))
         (v32 (logand (u64 (arg-imm args)) #xFFFFFFFF))
         (result (ash v32 (- shift))))
    (set-reg vm (arg-ra args) (sign-extend result 4))
    :continue))
...
(definstruction :shar-r-imm-alt-32 (vm args)
  (let* ((shift (mod (reg vm (arg-rb args)) 32))
    ...
    (set-reg vm (arg-ra args) (zn-inv 8 shifted))
    :continue))
...
(definstruction :cmov-iz-imm (vm args)
  (when (zerop (reg vm (arg-rb args)))
    (set-reg vm (arg-ra args) (u64 (arg-imm args))))
  :continue)
...
(definstruction :cmov-nz-imm (vm args)
  (unless (zerop (reg vm (arg-rb args)))
    (set-reg vm (arg-ra args) (u64 (arg-imm args))))
  :continue)
...
(definstruction :add-imm-64 (vm args)
  (set-reg vm (arg-ra args) (u64 (+ (reg vm (arg-rb args)) (arg-imm args))))
  :continue)
...
(definstruction :shlo-l-imm-64 (vm args)
  (let* ((shift (mod (arg-imm args) 64)))
    (set-reg vm (arg-ra args) (u64 (ash (reg vm (arg-rb args)) shift)))
    :continue))
...
(definstruction :shlo-r-imm-64 (vm args)
  (let* ((shift (mod (arg-imm args) 64)))
    (set-reg vm (arg-ra args) (ash (reg vm (arg-rb args)) (- shift)))
    :continue))
...
(definstruction :shar-r-imm-64 (vm args)
  (let* ((shift (mod (arg-imm args) 64))
         (signed64 (zn 8 (reg vm (arg-rb args))))
...
(definstruction :neg-add-imm-64 (vm args)
  (set-reg vm (arg-ra args) (u64 (- (arg-imm args) (reg vm (arg-rb args)))))
  :continue)
...
(definstruction :shlo-l-imm-alt-64 (vm args)
  (let ((shift (mod (reg vm (arg-rb args)) 64)))
    (set-reg vm (arg-ra args) (u64 (ash (u64 (arg-imm args)) shift)))
    :continue))
...
(definstruction :shlo-r-imm-alt-64 (vm args)
  (let ((shift (mod (reg vm (arg-rb args)) 64)))
    (set-reg vm (arg-ra args) (ash (u64 (arg-imm args)) (- shift)))
    :continue))
...
(definstruction :shar-r-imm-alt-64 (vm args)
  (let* ((shift (mod (reg vm (arg-rb args)) 64))
         (signed64 (zn 8 (u64 (arg-imm args))))
         (shifted (ash signed64 (- shift))))
    (set-reg vm (arg-ra args) (zn-inv 8 shifted))
    :continue))
...
(definstruction :rot-r-64-imm (vm args)
  (let* ((n (mod (arg-imm args) 64))
         (v (reg vm (arg-rb args))))
    (if (zerop n)
        (set-reg vm (arg-ra args) v)
        (set-reg vm (arg-ra args)
                 (u64 (logior (ash v (- n))
                              (ash v (- 64 n))))))
    :continue))
...
(definstruction :rot-r-64-imm-alt (vm args)
  (let* ((n (mod (reg vm (arg-rb args)) 64))
         (v (u64 (arg-imm args))))
    (if (zerop n)
        (set-reg vm (arg-ra args) v)
        (set-reg vm (arg-ra args)
...
(definstruction :rot-r-32-imm (vm args)
  (let* ((n (mod (arg-imm args) 32))
         (v (logand (reg vm (arg-rb args)) #xFFFFFFFF)))
...
(definstruction :rot-r-32-imm-alt (vm args)
  (let* ((n (mod (reg vm (arg-rb args)) 32))
         (v (logand (u64 (arg-imm args)) #xFFFFFFFF)))
    (let ((result (if (zerop n) v
                      (logior (ash v (- n))
                              (logand (ash v (- 32 n)) #xFFFFFFFF)))))
      (set-reg vm (arg-ra args) (sign-extend result 4))
...
(definstruction :add-32 (vm args)
  (set-reg vm (arg-rd args)
           (sign-extend (logand (+ (reg vm (arg-ra args))
                                   (reg vm (arg-rb args)))
                                #xFFFFFFFF)
                        4))
  :continue)
...
(definstruction :sub-32 (vm args)
  (let ((a (reg vm (arg-ra args)))
        (b (logand (reg vm (arg-rb args)) #xFFFFFFFF)))
    (set-reg vm (arg-rd args)
...
(definstruction :div-u-32 (vm args)
  (let ((a32 (logand (reg vm (arg-ra args)) #xFFFFFFFF))
        (b32 (logand (reg vm (arg-rb args)) #xFFFFFFFF)))
    (if (zerop b32)
        (set-reg vm (arg-rd args) +u64-max+)
        (set-reg vm (arg-rd args)
...
(definstruction :div-s-32 (vm args)
  (let ((a (zn 4 (logand (reg vm (arg-ra args)) #xFFFFFFFF)))
        (b (zn 4 (logand (reg vm (arg-rb args)) #xFFFFFFFF))))
    (cond
      ((zerop b)
       (set-reg vm (arg-rd args) +u64-max+))
      ((and (= a #.(- (ash 1 31))) (= b -1))
       (set-reg vm (arg-rd args) (zn-inv 8 a)))
      (t
       (set-reg vm (arg-rd args) (zn-inv 8 (truncate a b)))))
    :continue))
...
(definstruction :rem-u-32 (vm args)
  (let ((a32 (logand (reg vm (arg-ra args)) #xFFFFFFFF))
        (b32 (logand (reg vm (arg-rb args)) #xFFFFFFFF)))
    (if (zerop b32)
        (set-reg vm (arg-rd args) (sign-extend a32 4))
        (set-reg vm (arg-rd args) (sign-extend (mod a32 b32) 4)))
    :continue))
  ...
  (let ((a (zn 4 (logand (reg vm (arg-ra args)) #xFFFFFFFF)))
    ...
    (cond
      ((zerop b)
       ...
       (set-reg vm (arg-rd args) (zn-inv 8 a)))
      ((and (= a #.(- (ash 1 31))) (= b -1))
       (set-reg vm (arg-rd args) 0))
      (t
       (set-reg vm (arg-rd args) (zn-inv 8 (rem a b)))))
    :continue))
...
(definstruction :shlo-l-32 (vm args)
  (let* ((a (reg vm (arg-ra args)))
         (n (mod (reg vm (arg-rb args)) 32))
         (result (logand (ash a n) #xFFFFFFFF)))
    (set-reg vm (arg-rd args) (sign-extend result 4))
    :continue))
...
(definstruction :shar-r-32 (vm args)
  (let* ((a-signed (zn 4 (logand (reg vm (arg-ra args)) #xFFFFFFFF)))
         (n (mod (reg vm (arg-rb args)) 32))
         (result (ash a-signed (- n))))
    (set-reg vm (arg-rd args) (zn-inv 8 result))
    :continue))
...
(definstruction :div-u-64 (vm args)
  (let ((a (reg vm (arg-ra args)))
        (b (reg vm (arg-rb args))))
    (if (zerop b)
        (set-reg vm (arg-rd args) +u64-max+)
        (set-reg vm (arg-rd args) (floor a b)))
    :continue))
...
(definstruction :rem-u-64 (vm args)
  (let ((a (reg vm (arg-ra args)))
    ...
    (if (zerop b)
        (set-reg vm (arg-rd args) a)
        (set-reg vm (arg-rd args) (mod a b)))
    :continue))
...
(definstruction :rem-s-64 (vm args)
  (let ((a (zn 8 (reg vm (arg-ra args))))
        (b (zn 8 (reg vm (arg-rb args)))))
    (cond
      ((zerop b)
       (set-reg vm (arg-rd args) (zn-inv 8 a)))
      ((and (= a #.(- (ash 1 63))) (= b -1))
...
(definstruction :shlo-l-64 (vm args)
  (let* ((a (reg vm (arg-ra args)))
         (n (mod (reg vm (arg-rb args)) 64)))
    (set-reg vm (arg-rd args) (u64 (ash a n)))
    :continue))
...
(definstruction :shlo-r-64 (vm args)
  (let* ((a (reg vm (arg-ra args)))
         (n (mod (reg vm (arg-rb args)) 64)))
    (set-reg vm (arg-rd args) (ash a (- n)))
    :continue))

;; 209 = shar_r_64: Z₈⁻¹(⌊Z₈(φ_A) ÷ 2^(φ_B mod 64)⌋)
(register-opcode 209 :shar-r-64 :reg-reg-reg 1)
(definstruction :shar-r-64 (vm args)
  (let* ((a-signed (zn 8 (reg vm (arg-ra args))))
         (n (mod (reg vm (arg-rb args)) 64)))
    (set-reg vm (arg-rd args) (zn-inv 8 (ash a-signed (- n))))
    :continue))
...
(definstruction :mul-upper-s-s (vm args)
  (let* ((a (zn 8 (reg vm (arg-ra args))))
         ...
         (full (* a b))
         (hi (ash full -64)))
    (set-reg vm (arg-rd args) (zn-inv 8 hi))
    :continue))
...
(definstruction :set-lt-s (vm args)
  (set-reg vm (arg-rd args)
           (if (< (zn 8 (reg vm (arg-ra args)))
                  (zn 8 (reg vm (arg-rb args))))
               1 0))
  :continue)
...
(definstruction :cmov-iz (vm args)
  (when (zerop (reg vm (arg-rb args)))
    (set-reg vm (arg-rd args) (reg vm (arg-ra args))))
  :continue)
...
(definstruction :rot-l-32 (vm args)
  (let* ((a (logand (reg vm (arg-ra args)) #xFFFFFFFF))
         (n (mod (reg vm (arg-rb args)) 32)))
...
(definstruction :xnor (vm args)
  (set-reg vm (arg-rd args)
           (u64 (lognot (logxor (reg vm (arg-ra args))
                                (reg vm (arg-rb args))))))
  :continue)
...
(definstruction :max-s (vm args)
  (set-reg vm (arg-rd args)
           (zn-inv 8 (max (zn 8 (reg vm (arg-ra args)))
                          (zn 8 (reg vm (arg-rb args))))))
  :continue)
...
(definstruction :min-s (vm args)
  (set-reg vm (arg-rd args)
           (zn-inv 8 (min (zn 8 (reg vm (arg-ra args)))
                          (zn 8 (reg vm (arg-rb args))))))
  :continue)
