...
(definstruction :move-reg (vm args)
  (set-reg vm (arg-ra args) (reg vm (arg-rb args)))
  :continue)
...
(definstruction :sbrk (vm args)
  (let* ((mem (pvm-memory vm))
         (size-raw (reg vm (arg-rb args)))
         (h (mem-heap-base mem)))
    (cond
      ;; No heap configured
      ((zerop h)
       (set-reg vm (arg-ra args) 0)
       :continue)
       ...
       (let* ((size (u32 size-raw))
         ...
         (cond
           ...
           (t
            ...
            (set-reg vm (arg-ra args) (u64 old-top))
            :continue)))))))
...
(definstruction :count-set-bits-64 (vm args)
  (set-reg vm (arg-ra args) (logcount (reg vm (arg-rb args))))
  :continue)
...
(definstruction :leading-zero-bits-32 (vm args)
  (let ((v (logand (reg vm (arg-rb args)) #xFFFFFFFF)))
    (set-reg vm (arg-ra args)
...
(definstruction :sign-extend-8 (vm args)
  (set-reg vm (arg-ra args)
           (zn-inv 8 (zn 1 (logand (reg vm (arg-rb args)) #xFF))))
  :continue)
