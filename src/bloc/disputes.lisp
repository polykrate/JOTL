;;;; bloc/disputes.lisp — ED (Disputes Extrinsic)
;;;; Gray Paper §10
...
(defun encode-vote (vote)
  ...
  (let ((vote-bool (getf vote :vote))
        ...
        (sig-bytes (ensure-bytes (getf vote :signature))))
...
(defun encode-verdict (verdict)
  ...
  (let ((target-bytes (ensure-bytes (getf verdict :target)))
        (age (getf verdict :age))
        (votes (getf verdict :votes)))
      (assert (= (length target-bytes) 32))
    (let ((expected-votes (super-majority)))
...
(defun encode-culprit (culprit)
  ...
  (let ((target-bytes (ensure-bytes (getf culprit :target)))
        (key-bytes (ensure-bytes (getf culprit :key)))
        (sig-bytes (ensure-bytes (getf culprit :signature))))
      (assert (= (length target-bytes) 32))
...
(defun encode-fault (fault)
  ...
  (let ((target-bytes (ensure-bytes (getf fault :target)))
        (vote (getf fault :vote))
        (key-bytes (ensure-bytes (getf fault :key)))
        (sig-bytes (ensure-bytes (getf fault :signature))))
      (assert (= (length target-bytes) 32))
        (assert (= (length key-bytes) 32))
          (assert (= (length sig-bytes) 64))
          (concatenate '(vector (unsigned-byte 8))
