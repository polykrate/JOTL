...
;;; The Option tag byte (0x00/0x01) is handled by :save / decode-epoch-mark.
...
(define-state-closure header
  ...
  (:encode-unsealed :memo
   (concatenate '(vector (unsigned-byte 8))
                ...
                (if epoch-mark                       ; HE (variable)
                    (funcall epoch-mark :encode)
                    #(0))
                (if tickets-mark                     ; HW (variable)
                    (funcall tickets-mark :encode)
                    #(0))
  ...
  (:encode :memo
   (concatenate '(vector (unsigned-byte 8))
                (self :encode-unsealed)
                (encode-signature-96 seal)))         ; HS (96)

  ;; ── Hash H(E(H)) ────────────────────────────────────────────
  (:hash :memo (blake2b-256 (self :encode)))
