              ...
              (judgement
               (decode-length-prefixed-sequence
                ...
                (lambda (o3 s3)
                  (let ((p3 s3))
                    (decode>> (o3 p3)
                      (v (decode-with-length o3 p3))     ; validator
                      (i (decode-e2 o3 p3))              ; index (E2)
                      (s (decode-fixed-bytes o3 p3 64))  ; signature (FIXED 64 bytes!)
                      (values (list v i s) (- p3 s3)))))
                pos2))
